    ...
    <div className="space-y-8 animate-fade-in-up">
      ...
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold">
          Welcome back, <span className="bg-gradient-to-r from-cyan-400 to-violet-400 bg-clip-text text-transparent">{user?.name}</span> 👋
        </h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map(({ label, value, icon: Icon, color, link }) => (
          <Link key={label} to={link} className="group">
            <div className="glass rounded-2xl p-5 hover:bg-white/[0.08] transition-all duration-300 border border-white/5 hover:border-white/10">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-sm text-[var(--color-text-dim)]">{label}</p>
                  <p className="text-3xl font-bold mt-2">{value}</p>
                </div>
                <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${color} flex items-center justify-center opacity-80 group-hover:opacity-100 group-hover:scale-110 transition-all`}>
      ...
      <div>
        <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
          <Sparkles className="w-5 h-5 text-amber-400" /> Quick Actions
        </h2>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <Link to="/stations" className="glass rounded-2xl p-6 flex flex-col items-center text-center gap-3 hover:bg-white/[0.08] transition-all border border-white/5 hover:border-cyan-500/30 group">
          ...
          <Link to="/vehicles" className="glass rounded-2xl p-6 flex flex-col items-center text-center gap-3 hover:bg-white/[0.08] transition-all border border-white/5 hover:border-violet-500/30 group">
          ...
          <Link to="/history" className="glass rounded-2xl p-6 flex flex-col items-center text-center gap-3 hover:bg-white/[0.08] transition-all border border-white/5 hover:border-emerald-500/30 group">
      ...
      <div>
        ...
        {recentBookings.length === 0 ? (
          <div className="glass rounded-2xl p-8 text-center border border-white/5">
            <Battery className="w-12 h-12 text-[var(--color-text-dim)] mx-auto mb-3 opacity-40" />
          ...
          <div className="space-y-3">
            {recentBookings.map(b => {
              ...
              return (
                <div key={b.id} className="glass rounded-xl p-4 flex items-center gap-4 border border-white/5">
